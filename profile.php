...
// SAFE DATABASE UPDATE: Tahimik na gagawa ng columns kung sakaling wala pa
@mysqli_query($conn, "ALTER TABLE users ADD COLUMN phone VARCHAR(50) NULL");
@mysqli_query($conn, "ALTER TABLE users ADD COLUMN avatar VARCHAR(255) NULL");
@mysqli_query($conn, "ALTER TABLE users ADD COLUMN pref_push TINYINT(1) DEFAULT 1");
@mysqli_query($conn, "ALTER TABLE users ADD COLUMN pref_email TINYINT(1) DEFAULT 1");
@mysqli_query($conn, "ALTER TABLE users ADD COLUMN pref_milestone TINYINT(1) DEFAULT 0");
@mysqli_query($conn, "ALTER TABLE users ADD COLUMN currency VARCHAR(10) DEFAULT 'PHP'");
@mysqli_query($conn, "ALTER TABLE users ADD COLUMN date_format VARCHAR(20) DEFAULT 'MM/DD/YYYY'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['update_profile_settings'])) {
        ...
        $curr_query = @mysqli_query($conn, "SELECT avatar FROM users WHERE user_id = '$user_id'");
        $curr_data = $curr_query ? mysqli_fetch_assoc($curr_query) : null;
        $avatar_path = $curr_data['avatar'] ?? '';
    ...
    elseif (isset($_POST['update_security'])) {
        ...
        $sec_query = @mysqli_query($conn, "SELECT password FROM users WHERE user_id='$user_id'");
        if ($sec_query && mysqli_num_rows($sec_query) > 0) {
            $sec_data = mysqli_fetch_assoc($sec_query);
            // Verify muna kung tama old password
            if (password_verify($curr_pass, $sec_data['password'])) {
...
// FETCH CURRENT USER DATA (Sino ang naka-login)
...
<body class="flex h-screen overflow-hidden bg-background relative">
  ...
  <div class="flex-1 flex flex-col h-full overflow-hidden relative z-10">
    ...
    <header class="h-24 flex items-center justify-center px-8 sm:px-10 shrink-0 z-10 border-b border-white/40 relative">
      ...
      <div class="text-center animate-fade-in-up">
        <h1 class="text-3xl font-bold font-title text-slate-800">Settings</h1>
        <p class="text-muted-foreground text-sm font-medium mt-1">Manage your account preferences and security.</p>
      </div>
      
      <div class="absolute right-8 flex items-center gap-5">
    ...
    <main class="flex-1 overflow-auto p-6 sm:p-8">
      ...
      <div class="dashboard-card !p-0 overflow-hidden flex flex-col md:flex-row min-h-[500px] animate-fade-in-up delay-100 border-0 mt-2">
        ...
        <div class="w-full md:w-72 border-b md:border-b-0 md:border-r border-white/40 p-6 bg-white/20">
          <nav class="space-y-2">
        ...
        <div class="flex-1 p-6 sm:p-10 relative">
          ...
          <div id="tab-security" class="tab-content hidden animate-fade-in-up">
            ...
            <div class="pt-8 border-t border-white/60 max-w-2xl">
              ...
              <button type="button" class="px-6 py-3 text-sm font-bold text-blue-600 bg-white/80 border border-white hover:bg-white hover:shadow-sm rounded-xl transition-all">
                Enable 2FA
              </button>
            </div>
          </div>

...
if (isset($_GET['token'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($new_password === $confirm_password) {
                ...
                if (mysqli_query($conn, $update_query)) {
                    $message = "<div class='bg-emerald-100/80 border border-emerald-200 text-emerald-700 p-4 rounded-xl text-sm font-medium mb-6 text-center backdrop-blur-sm'>Password updated successfully! <br><br> <a href='index.php' class='underline font-bold hover:text-emerald-900 transition-colors'>Click here to login</a></div>";
                    $valid_token = false; 
                } else {
        ...
        $message = "<div class='bg-rose-100/80 border border-rose-200 text-rose-700 p-4 rounded-xl text-sm font-medium mb-6 text-center backdrop-blur-sm'>Invalid or expired reset token. <br><br> <a href='forgot_password.php' class='underline font-bold hover:text-rose-900 transition-colors'>Request a new link</a></div>";
    }
} else {
    $message = "<div class='bg-rose-100/80 border border-rose-200 text-rose-700 p-4 rounded-xl text-sm font-medium mb-6 text-center backdrop-blur-sm'>No reset token provided. <br><br> <a href='forgot_password.php' class='underline font-bold hover:text-rose-900 transition-colors'>Go to Forgot Password page</a></div>";
}
?>
  ...
  <div class="flex h-screen w-full items-center justify-center p-4 lg:p-8 relative z-10">
    ...
    <div class="w-full max-w-5xl h-full max-h-[850px] login-form-card flex flex-col lg:flex-row overflow-hidden shadow-[0_20px_60px_rgba(37,99,235,0.15)]">
      ...
      <div class="hidden lg:flex w-1/2 bg-gradient-to-br from-white/30 to-blue-50/20 p-12 flex-col justify-center items-center relative border-r border-slate-200/40">
        ...
        <div class="relative w-full max-w-xs mt-10 h-64 flex justify-center items-center left-anim">
            <div class="absolute z-10 glass-icon-card p-6 rounded-2xl shadow-xl float-anim" style="animation-delay: 0s;">
                <i class="fa-solid fa-lock text-5xl text-blue-600 drop-shadow-md"></i>
            </div>
            ...
            <div class="absolute top-12 right-4 glass-icon-card p-3 rounded-xl shadow-md float-anim" style="animation-delay: 1.8s;">
                <i class="fa-solid fa-fingerprint text-2xl text-rose-400"></i>
            </div>
             <div class="absolute bottom-12 left-4 glass-icon-card p-3 rounded-xl shadow-md float-anim" style="animation-delay: 2.5s;">
                <i class="fa-solid fa-user-shield text-2xl text-cyan-500"></i>
            </div>
        </div>

        <div class="mt-12 text-center left-anim">
          <h2 class="brand-font text-2xl font-bold mb-3 text-slate-800">Secure your account.</h2>
          <p class="text-slate-600 font-medium text-sm leading-relaxed max-w-xs mx-auto">
            Update your credentials and continue tracking your financial journey safely.
          </p>
        </div>
      ...
      <div class="w-full lg:w-1/2 p-8 lg:p-12 flex flex-col bg-white/20 right-anim form-scroll-container">
        ...
        <div class="mb-8 text-center lg:text-left pt-2 lg:pt-8">
          <h2 class="text-3xl font-bold mb-2 text-slate-900">Reset Password</h2>
          <p class="text-slate-600 font-medium">Create a new secure password.</p>
        </div>

        <?php echo $message; ?>

        <?php if ($valid_token): ?>
        <form method="POST" action="" class="space-y-5 pb-8">
